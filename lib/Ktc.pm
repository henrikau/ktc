...
sub processArguments {
    ...
    push @{$self->{KTCLIBAVTP}}, "$lib/libtimedc_avtp.$self->{LIBEXT}";
    push @{$self->{KTCLIBAVTP}}, "$lib/libmrp.$self->{LIBEXT}";
    return $self;
}
...
sub link_after_cil {
	...
	if($self->{FREERTOS} == 1){
		...
		push @libs, "-lrt", "-lpthread", "-ltimedc_avtp", "-lmrp", "-";
	}
